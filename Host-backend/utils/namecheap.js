...
namecheap.prototype = {
    ...
    return {
      ...
      getPricing: function(type, code, callback) {
        ...
        if('[object Function]' == Object.prototype.toString.call(code))
          ...
          params.PromotionCode = code;
          if(type?.ProductName){
            params.ProductName = type.ProductName;
          }
          if(type?.ActionName){
            params.ActionName = type.ActionName;
          }
  ...
  command: function(cmd, params, callback, method) {
    ...
    request(options, function(err, res, body) {
        body = parser.xml2js(body, {compact: true, spaces: 0});
        err = body?.ApiResponse?.Errors?.Error;
        res = body?.ApiResponse?.CommandResponse;
        err = err ? { code : err._attributes.Number, message: err._text } : undefined;
